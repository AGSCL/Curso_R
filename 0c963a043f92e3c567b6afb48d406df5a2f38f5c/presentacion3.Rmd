...
knitr::include_graphics("G:/Mi unidad/Alvacast/Curso/gastos_casa_csv_foto.png")
```
...
- Los elementos que definen a una importación de un excel (*.xlsx) que hay que tener en consideración son los siguientes:
  ...
  -Si quiere abrir un **visor de datos (View())**

## Configuración de Datos, p2 {.future-steps .smaller}

```{r cargar_xlsx, echo=T, warning=F, size = 'tiny'}
library(readxl)
coronavirus <- read_excel("G:/Mi unidad/Alvacast/Curso/0c963a043f92e3c567b6afb48d406df5a2f38f5c/coronavirus.xlsx", 
    sheet = "Hoja1")
str(coronavirus)
```
...
plot(coronavirus$infectados,coronavirus$monto_invertido_med, xlab="Infectados",ylab="Monto Invertido Med.")
```
...
coronavirus$mes<-as.numeric(coronavirus$mes) 
coronavirus$estado<-as.factor(coronavirus$estado) 

library(reshape2)
dcast(coronavirus, estado ~ mes, value.var="infectados")
dcast(coronavirus, estado ~ mes, value.var="monto_invertido_med")
```
...
- Tutorialspoint. Mean, Median and Mode. http://datos.gob.cl/uploads/recursos/Capacitaciones_Prevencion.csv

## Ejercicio {#future-steps .smaller}

- Acceda a la base de datos de Total de Capacitaciones en Prevención de Riesgos Laborales, Instituto de Seguridad Laboral ISL
- Previsualicemos los datos: http://datos.gob.cl/dataset/9010/resource/5edb0069-43e7-451e-8717-5f8444056641
- Generemos una carpeta en escritorio, de nombre "Cap_RL"
- Genere un proyecto con el mismo nombre
- Cargarla en R y conviertala en un data frame, asignándole el siguiente nombre "cap_prev".
- obtener la media y la mediana por sexo (2da y 3ra columnas)
- Obtenga la media y la mediana sin el total (fila 15)
- ¿Qué sexo en promedio obtuvo más capacitaciones en el primer mes?
...
- Cargarla en RStudio, y enviar a gonzalez.santacruz.andres@gmail.com, un gráfico de dispersión de decimal1 en contraste con decimal2. Siguiendo la misma estructura, reemplace el vector decimal1 por el vector sexo, pero en formato factor) y adjúntelo al correo.
