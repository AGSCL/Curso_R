...
did.reg <- plm(conf ~ txtime + met + txtime:met + Primera, 
               data = covid19_chile_vacuna_vs_confirmados,  index=c("Region", "fecha"), model = "within")
...
Produc_means <- covid19_chile_vacuna_vs_confirmados %>% 
  dplyr::group_by(Region) %>% 
  transmute(y_median = median(conf),
            y = conf, 
            fecha = fecha) %>% 
  dplyr::ungroup() %>% 
...
reg_num <-
cbind.data.frame(region=unique(covid19_chile_vacuna_vs_confirmados$Region),
                 numreg=1:length(unique(covid19_chile_vacuna_vs_confirmados$Region)))

covid19_chile_vacuna_vs_confirmados_num <-
  covid19_chile_vacuna_vs_confirmados %>% 
      dplyr::mutate(numfech=as.numeric(fecha)-min(as.numeric(fecha))) %>% 
      dplyr::mutate(numfech=numfech+1) %>% 
      dplyr::left_join(reg_num, by=c("Region"="region")) %>% 
      dplyr::mutate(numreg=as.numeric(numreg)) %>% 
      dplyr::mutate(punto=factor(ifelse(txtime==1 & met==1,1,0))) %>% 
  as.data.frame()


ggplot(covid19_chile_vacuna_vs_confirmados_num, aes(fecha, conf, color = met)) +
