...
#Obtuvimos los errores de estándar robustos a variaciones por Región
coeftest(did.reg, vcov = function(x) 
  vcovHC(x, cluster = "group", type = "HC1")) %>% 
   broom::tidy() %>% 
  #Cambiamos los nombres de las variables
  dplyr::mutate(term= dplyr::case_when(term=="txtime1"~"14 de Febrero hasta el día de obtención",
                                       term=="Primera_100mil"~"Primera dosis por cada 100 mil hab.",
                                       term=="PCR_100mil"~"PCR's por cada 100 mil hab.",
                                       term=="Hosp_UCI_100mil"~"Hospitalizaciones por cada 100 mil hab.",
                                       term=="txtime1:met1"~"Coquimbo x 14 de Febrero"
                                       )) %>% 
  dplyr::mutate_at(vars("estimate","std.error","statistic"), funs(round(.,2))) %>% 
  #Aproximamos a 3, de manera que los interesados puedan ver el valor p con 3 decimales
  dplyr::mutate_at(vars("p.value"), funs(round(.,3))) %>% 
  dplyr::mutate(p.value=ifelse(p.value<0.001,"<0.001",as.character(p.value))) %>% 
  ...
  knitr::kable(.,format = "html", format.args = list(decimal.mark = ".", big.mark = ","),
               ...
               col.names = c("Variables Independientes","Estimador","Error Estándar","Estadístico t", "Sig"),
align =c('l',rep('c', 101)))%>%
  #destaca la quinta fila correspondiente a la intervención para el tratado
  kableExtra::row_spec(5,bold=T,hline_after = T) %>% 
    #kableExtra::add_footnote(c("Nota. txtime= 14 de Febrero; Primera=Número de primeras vacunas aplicadas; met= Coquimbo"), notation = "none")%>%
...
- A partir del gráfico no es tan claro si las diferencias se deben al 14 de febrero
- Adicionalmente, la capacidad de predicción del modelo se ve baja
...
- Hay patrones temporales que no son capturados (ciclos, tendencias, estaciones)
