...
::: notes
Comentarios:
- Fuentes de sesgo: Selección/Representación (Cobertura, Muestreo - estatus de elegibilidad, reemplazo, etc.-, No-respuesta, ajuste post-encuesta - ej., ponderadores-), datos perdidos, error de medición (validez, medición -ej., si una mujer pregunta a hombres si es sexista-, procesamiento - ej., tipeo-), selección del modelo, seguimiento (follow-up), confusores, comparaciones múltiples (mientras queramos comparar en más atributos, es más fácil encontrar falsos positivos: mientras más test e hacen en una muestra, más probable es encontrar un hallazgo), 
:::
...
align =c('l',rep('c', 101)))%>%
  ...
  kableExtra::add_footnote(c("Nota. IC 95%= Intervalos de Confianza al 95%; Datos obtenidos al ", format(Sys.time(), '%d %B, %Y')), notation = "none")%>%
