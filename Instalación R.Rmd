 ...
 **Reinicie el computador una vez finalizada la instalación**
 ...
 Posteriormente, ingrese a Rstudio e instale 3 paquetes mediante el siguiente comando `install.packages(c("dplyr","ggplot2", "data.table"))`
...
Habiendo cumplido con el paso previo, abra R Studio y corra el siguiente comando en la Consola: `install.packages(c("dplyr", "ggplot2","data.table"))`. Si los paquetes se instalan y usted puede correr el siguiente comando `require(dplyr);data(mtcars);mtcars%>% filter(vs==0, gear==5, carb==4) %>% select(mpg)`, asegúrese que su resultado debiese coincidir con el siguiente número: `r require(dplyr);data(mtcars);mtcars%>% filter(vs==0, gear==5, carb==4) %>% select(mpg)`.
